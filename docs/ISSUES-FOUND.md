...
- **Suspected cause:** the malware-scan/quarantine heuristics in the incident-response/threat path are too broad over `vendor/` trees. Related to the `threat_service` false-positive tuning (2026-07-04 decisions log) but for the **cleanup/quarantine** path, not just detection. Ties to open task #2 ("recurse the whole account home") — recursion is correct, but the quarantine *criteria* are too aggressive.
- **Repro:** scan a stock Laravel app with `intervention/image` installed; assert `AbstractDecoder.php` + `symfony-ghost.svg.php` are NOT quarantined. Dev Door dry-run of the `security-incident` skill on a vendor tree should reproduce the over-broad plan.
