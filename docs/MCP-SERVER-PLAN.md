...
| **3 ◑ (in progress)** | **Bounded write tools** — every write EXECUTE-gated (Rule 7; a viewer can never execute), 0 AI actions, no shell/delete/restore. Shipped 2026-07-23: run_security_scan + run_threat_scan (inline, persist like the router — read-only probes), run_playbook (start+poll: creates a durable PlaybookRun + enqueues run_playbook_task, returns run_id immediately — §7) + get_playbook_run (poll status/output). Validated live: the write-gate refuses a non-owner; run_playbook rejects an unknown slug before enqueuing; a real run_security_scan on panel2 → grade D, 19 findings, persisted. **Remaining:** create_site/issue_ssl/create_database (CyberPanel CLI, procedure-carrying — create_site already verifies the domain appears before success, §3a) + run_backup (existing job only). Deferred: skills as MCP prompts, injection framing on results | Claude runs a playbook end-to-end via poll; a 6-minute op does not hit the 5-min timeout | 3–4 d |
