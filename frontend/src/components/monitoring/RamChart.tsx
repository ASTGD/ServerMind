  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-xs shadow-lg">
      ...
      <p className="font-semibold text-foreground">
        RAM: {pct != null ? `${pct.toFixed(1)}%` : "—"}
      </p>
    </div>
  )
}

export default function RamChart({ data, height = 180 }: Props) {
  const lastValue = data.length ? (data[data.length - 1].ram_percent ?? 0) : 0
  const color = colorForValue(Number(lastValue))

  const chartData = data.map((p) => ({
    time: p.recorded_at,
    ram: p.ram_percent != null ? +Number(p.ram_percent).toFixed(1) : null,
  }))
