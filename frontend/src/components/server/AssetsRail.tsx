...
/** The Assets companion rail — wrapped in one tinted container so it reads as a
 *  distinct sidebar region rather than a loose stack of cards, matching the
 *  Dashboard's tighter card density (rounded-xl / border / bg-card / p-4).
 *  At-a-glance fleet pulse, dedicated Hosting and Cloud stat cards (with
 *  add/connect empty states), and plan usage. Runs on data we already have
...
export default function AssetsRail({ servers, cloudAccounts, onFilter, onAddHosting, onConnectCloud }: Props) {
  ...
  return (
    <aside className="w-80 shrink-0 rounded-2xl bg-muted/40 p-3">
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-3 grid grid-cols-3 gap-3 text-center">
            <div>
              <p className="text-[22px] font-semibold tabular-nums text-foreground">{total}</p>
              <p className="mt-0.5 text-xs text-muted-foreground">Total</p>
            </div>
            <div>
              <p className="text-[22px] font-semibold tabular-nums text-green-600 dark:text-green-500">{online}</p>
              <p className="mt-0.5 text-xs text-muted-foreground">Online</p>
            </div>
            <div>
              <p className={`text-[22px] font-semibold tabular-nums ${attention > 0 ? "text-amber-500" : "text-foreground"}`}>{attention}</p>
          ...
          {attention > 0 && (
            <button
              ...
              className="group mt-3 flex w-full items-center gap-2 rounded-lg bg-amber-500/10 px-3 py-2 text-left text-xs text-amber-700 transition-colors hover:bg-amber-500/15 dark:text-amber-400"
            >
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {panels > 0 ? (
            <div className="mt-3 flex items-baseline gap-2">
              <span className="text-[22px] font-semibold tabular-nums text-foreground">{panels}</span>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {cloudAccounts.length > 0 ? (
            <div className="mt-3 flex gap-8">
              <div>
                <p className="text-[22px] font-semibold tabular-nums text-foreground">{cloudAccounts.length}</p>
                <p className="mt-0.5 text-xs text-muted-foreground">{cloudAccounts.length === 1 ? "Account" : "Accounts"}</p>
              </div>
              <div>
                <p className="text-[22px] font-semibold tabular-nums text-foreground">{totalImported}</p>
